...
for file in range(len(data)):
    ...
    while pid_arr != {}:
        ...
        sys.stdout.flush()
    print("complete!")
    np.savez(filename[0] + "_results_arr", results_arr)
    

    # Splitting the vlbi_cal from the obs number and the year
    file_num_date = filename[0].rsplit(".", 1)
    ...
    ### Getting heat maps
    plt.imshow(1-np.mean(results_sum>0.5,axis=0), extent=np.array([kd_range, ki_range]).flatten(), origin='lower', cmap='gray')
    plt.title('Low PhEff Optimization')
    plt.xlabel('k_d')
    plt.ylabel('k_i')
    plt.savefig(file_num_date + '_dvi', dpi=300, format = 'jpg')
    #plt.show()


    plt.imshow(1-np.mean(results_sum>0.5,axis=1), extent=np.array([kd_range, kp_range]).flatten(), origin='lower', cmap='gray')
    plt.title('Low PhEff Optimization')
    plt.xlabel('k_d')
    plt.ylabel('k_p')
    plt.savefig(file_num_date + '_dvp', dpi=300, format = 'jpg'))
    #plt.show()


    plt.imshow(1-np.mean(results_sum>0.5,axis=2), extent=np.array([ki_range, kp_range]).flatten(), origin='lower', cmap='gray')
    plt.title('Low PhEff Optimization')
    plt.xlabel('k_i')
    plt.ylabel('k_p')
    plt.savefig(file_num_date + '_pvi', dpi=300, format = 'jpg')
    #plt.show()